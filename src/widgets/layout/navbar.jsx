...
export function Navbar({ brandName, routes, action }) {
  ...
  const navList = (
    <ul className="mb-4 mt-2 mr-40 flex flex-col gap-2 text-inherit lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      {routes.filter((route) => route.name !== 'Profile').map(({ name, path, icon, href, target }) => (
        <Typography
          key={name}
          as="li"
          variant="small"
        ...
        >
          {href ? (
            ...
            <Link
              to={path}
              target={target}
              className={`flex items-center gap-1 p-1 font-normal ${(!isHome && name === 'Contact') ? 'pointer-events-none opacity-50' : ''}`}
              onClick={() => {
                if (name === 'Contact') {
